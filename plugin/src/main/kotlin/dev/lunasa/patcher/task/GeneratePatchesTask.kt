...
abstract class GeneratePatchesTask : DefaultTask() {
    ...
    fun generatePatches() {
        ...
        val time = measureTime {
            originalDir.walk().forEach { originalFile ->
                if (originalFile.isFile) {
                    val relativePath = originalDir.toPath()
                        .relativize(File(originalFile.parentFile, "${originalFile.name}.patch").toPath())
        ...
        lifecycle(logger, "Generated $patchCount patches in $time", "diff")
    }
}
